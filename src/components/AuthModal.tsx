...
export function AuthModal({ isOpen, onClose, defaultRole = 'renter' }: AuthModalProps) {
  const [isLogin, setIsLogin] = useState(true);
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [name, setName] = useState('');
  const [role, setRole] = useState<'renter' | 'landlord'>(defaultRole);
      ...
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl relative">
        ...
        <div className="p-8">
          ...
          <p className="text-gray-500 mb-6">
            {isLogin ? 'Enter your details to sign in.' : 'Join us to find or list your next home.'}
          </p>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6">
              <button
                ...
                disabled={loading}
                className="w-full flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
              >
